    ...
    public class ProjectTask : Activity
    {
        ...
        public void SetProgress(int progress)
        {
            if (progress > 100 || progress < 0)
                throw new InvalidActivityProgressException();
            if (tasks.Any())
                throw new InvalidActivityProgressException();
        ...
        public int CalculateProgress()
        {
            ...
            float currentProgressEffort = 0;
            float expectedEffort = 0;
            foreach (ProjectTask task in tasks)
            {
                float effort = task.CalculateExpectedEffort();
                currentProgressEffort += effort * task.CalculateProgress();
                expectedEffort += effort;
            }

            return Convert.ToInt32(currentProgressEffort / expectedEffort);
        }
    }
}
